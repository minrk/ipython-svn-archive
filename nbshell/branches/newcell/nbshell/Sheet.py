...
class Sheet(object):
    ...
    def Update(self, update = True, celllist = False, dicts = False, output = False):
        ...
        if output:
            ...
            for logid in logs.keys():
                for cell in self.notebook.cells: #new does not support (multiple) logs yet. (notebook.Cell(x) for x in logs[logid].log):
    ...
    def DefaultSheet(self , update = True):
        ...
        self.element = self.notebook.newdefault_sheet()
...
Please use Return to insert new lines and Shift-Return to execute inputs. """
    ...
    def InsertElement(self, block, type, cell, pos = None, update = True):
        ...
        #dbg
        print "SHEET Inserting with number", number, block, type, cell, pos

        element = etree.Element('ipython-cell',type = type, number = str(number)) #NBDOC: notebook.Cell() to handle this? there is Cell.get_sheet_tags..
    ...
    def UpdateOutput(self, logid, cell, update = True, block=None, pos = None):
        ...
        #cell.update() #should be always up-to-date
...
#            self.__update_list(blocklist)
    ...
    def InsertCode(self, block, pos, logid = None, update = True):
        ...
        def insert(p):
            ...
            #XXX use cell.get_sheet_tag-some-new-variant for these (?)
