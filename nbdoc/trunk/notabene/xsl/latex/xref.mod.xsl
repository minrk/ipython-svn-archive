...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" exclude-result-prefixes="doc" version="1.0">
	...
	<xsl:template match="anchor">
		...
		<xsl:text>}{}</xsl:text>
	</xsl:template><xsl:key name="cross-refs" match="xref|link" use="@linkend"/><xsl:template name="id.is.xrefed">
    ...
    </xsl:template><xsl:template match="xref|link">
...
</xsl:template><xsl:template name="generate.xref.text">
	...
	</xsl:template><xsl:template name="generate.hyperlink">
		<xsl:param name="target"/>
		<xsl:param name="text"/>
	...
	</xsl:template><xsl:template match="*" mode="insert.label.markup" name="insert.label.markup">
		<xsl:param name="id" select="@id"/>
		<xsl:choose>
    ...
    </xsl:template><xsl:template match="title" mode="insert.label.markup">
    ...
    </xsl:template><xsl:template match="*" mode="insert.title.markup" name="generate.title.markup">
	...
	</xsl:template><xsl:template match="title" mode="insert.title.markup">
		<xsl:param name="title"/>
		<xsl:param name="is.component">
			<xsl:choose>
				<xsl:when test="contains(local-name(..), 'info')">
	...
	</xsl:template><xsl:template match="book" mode="insert.title.markup">
		<xsl:param name="title"/>
	...
	</xsl:template><xsl:template match="ulink" name="ulink">
    ...
    </xsl:template><xsl:template match="olink">
		<xsl:apply-templates/>
    </xsl:template><xsl:template match="*" name="title.xref">
	<xsl:param name="target" select="."/>
	<xsl:choose>
	    <xsl:when test="name($target) = 'figure'   or name($target) = 'example'   or name($target) = 'equation'   or name($target) = 'table'   or name($target) = 'dedication'   or name($target) = 'preface'   or name($target) = 'bibliography'   or name($target) = 'glossary'   or name($target) = 'index'   or name($target) = 'setindex'   or name($target) = 'colophon'">
		<xsl:call-template name="gentext.startquote"/>
    ...
    </xsl:template><xsl:template name="generate.typeset.url">
	...
	</xsl:template><xsl:template name="generate.string.url">
		<xsl:param name="hyphenation"/>
...
</xsl:template><xsl:template match="*" mode="xref-to">
  <xsl:param name="referrer"/>
  <xsl:param name="xrefstyle"/>
...
</xsl:template><xsl:template match="abstract|article|authorblurb|bibliodiv|bibliomset                      |biblioset|blockquote|calloutlist|caution|colophon                      |constraintdef|formalpara|glossdiv|important|indexdiv                      |itemizedlist|legalnotice|lot|msg|msgexplan|msgmain                      |msgrel|msgset|msgsub|note|orderedlist|partintro                      |productionset|qandadiv|refsynopsisdiv|segmentedlist                      |set|setindex|sidebar|tip|toc|variablelist|warning" mode="xref-to">
  <xsl:param name="referrer"/>
  <xsl:param name="xrefstyle"/>
...
</xsl:template><xsl:template match="dedication|preface|chapter|appendix" mode="xref-to">
  <xsl:param name="referrer"/>
  <xsl:param name="xrefstyle"/>
...
</xsl:template><xsl:template match="glossterm" mode="xref-to">
  <xsl:apply-templates/>
...
</xsl:template><xsl:template match="section|simplesect                      |sect1|sect2|sect3|sect4|sect5                      |refsect1|refsect2|refsect3" mode="xref-to">
  <xsl:param name="referrer"/>
  <xsl:param name="xrefstyle"/>
...
</xsl:template><xsl:template match="qandadiv" mode="xref-to">
  <xsl:param name="referrer"/>
  <xsl:param name="xrefstyle"/>
...
</xsl:template><xsl:template match="para" mode="xref-to">
  <xsl:param name="referrer"/>
  <xsl:param name="xrefstyle"/>
