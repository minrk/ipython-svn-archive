...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" exclude-result-prefixes="doc" version="1.0">
...
</xsl:template><xsl:template match="qandaset/qandaentry">
		<xsl:text>\vspace{1em}
</xsl:text>
		<xsl:text>\noindent{}</xsl:text>
	...
	</xsl:template><xsl:template match="qandadiv">
	<!-- get the preamble -->
...
<!-- pseudo table of contents -->
	...
	<xsl:text>\vspace{1em}
...
</xsl:template><xsl:template match="question">
		...
		<xsl:call-template name="process.qanda.toc"/>
	</xsl:template><xsl:template match="qandadiv/title" mode="qandatoc.mode">
		<xsl:variable name="qalevel">
			<xsl:call-template name="qandadiv.section.level"/>
		</xsl:variable>
		<xsl:call-template name="label.id">
		...
		<xsl:apply-templates select="." mode="label.markup"/>
		<xsl:text> </xsl:text>
	</xsl:template><xsl:template match="answer" mode="qandatoc.mode"/><xsl:template match="revhistory" mode="qandatoc.mode"/><xsl:template name="question.answer.label">
	<!-- variable: deflabel -->
  <xsl:variable name="deflabel">
