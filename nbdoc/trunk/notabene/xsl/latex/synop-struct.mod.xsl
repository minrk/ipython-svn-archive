...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" exclude-result-prefixes="doc" version="1.0">
	...
	<xsl:apply-templates/>
	</xsl:template><xsl:template match="cmdsynopsis">
	<xsl:call-template name="label.id"/>
	<xsl:text>
\begin{list}{}{\setlength{\itemindent}{-\leftmargin}\setlength{\parsep}{0mm}}
	...
	<xsl:apply-templates/>
	</xsl:template><xsl:template match="cmdsynopsis//replaceable" priority="2">
    ...
    </xsl:template><xsl:template match="funcprototype">
	<xsl:apply-templates/>
	<xsl:if test="$funcsynopsis.style='kr'">
	    <xsl:apply-templates select="./paramdef" mode="kr-funcsynopsis-mode"/>
	</xsl:if>
    </xsl:template><xsl:template match="funcdef">
	<xsl:apply-templates/>
    </xsl:template><xsl:template match="funcdef/function">
	<xsl:choose>
    ...
    </xsl:template><xsl:template match="void">
	<xsl:choose>
    ...
    </xsl:template><xsl:template match="varargs">
	<xsl:text>( ... );</xsl:text>
    </xsl:template><xsl:template match="paramdef">
	<!-- VAR paramnum -->
    ...
    </xsl:template><xsl:template match="paramdef/parameter">
	<xsl:choose>
	    <xsl:when test="$funcsynopsis.decoration != 0">
		<xsl:apply-templates/>
	    </xsl:when>
	...
	<xsl:apply-templates/>
	<xsl:text>;</xsl:text>
    </xsl:template><xsl:template match="funcparams">
	<xsl:text>(</xsl:text>
	<xsl:apply-templates/>
	<xsl:text>)</xsl:text>
