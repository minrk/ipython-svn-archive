...
from notabene.xmlutils import nsmap, ip
...
class Executor(object):
    ...
    def run_all(self, oldlog, newlog):
        ...
        for cell in cells:
            newcell = ET.SubElement(newlog, ip.cell, number=str(cell.number))
    ...
    def rerun_since(self, log, number):
        ...
        cells = sorted((Cell(x) for x in
            oldlog.xpath('./ip:cell[@number>=%s]' % number,
                namespaces=nsmap(ip))),
    ...
    def vacuum(self, nb):
        ...
        for sheet in sheets:
            ...
            for logid in logids:
                ...
                cells = sheet.xpath('.//ip:block[@logid="%s"]/ip:cell' %
                    logid, namespaces=nsmap(ip))
    ...
    def contiguify(self, log, sheets=None):
        ...
        """
        numbers = sorted(int(x) for x in log.xpath('./ip:cell/@number',
            namespaces=nsmap(ip)))
        old2new = {}
