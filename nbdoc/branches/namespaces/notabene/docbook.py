...
from notabene.xmlutils import nsmap, strip_ns, ip, db
from notabene.PyFontify import fontify
...
class DBFormatter(Formatter):
    ...
    def transform_stdout(self, elem, number):
        text = transform_text(elem.text)
        logid = elem.xpath("../../@id")[0]
        wholetext = ('<db:anchor id="%s-stdout%s"/>%s' % 
                        (logid, number, text))
        return wholetext
    ...
    def transform_stderr(self, elem, number):
        text = transform_text(elem.text)
        logid = elem.xpath("../../@id")[0]
        wholetext = ('<db:anchor id="%s-stderr%s"/>%s' % 
                        (logid, number, text))
        return wholetext
    ...
    def transform_equation(self, equ):
        ...
        dbeq = ET.Element(db.informalequation)
        tex = equ.text
        verbatim = equ.get('verb', None)
        if verbatim != '1':
            tex = r'$$%s$$' % tex
        mo = ET.SubElement(dbeq, db.mediaobject)
        to = ET.SubElement(mo, db.textobject)
        phrase = ET.SubElement(to, db.phrase, role="latex")
        phrase.text = tex
        dbeq.tail = equ.tail
        return dbeq

    def transform_inlineequation(self, equ):
        dbeq = ET.Element(db.inlineequation)
        tex = equ.text
        verbatim = equ.get('verb', None)
        if verbatim != '1':
            tex = r'$%s$' % tex
        mo = ET.SubElement(dbeq, db.inlinemediaobject)
        to = ET.SubElement(mo, db.textobject)
        phrase = ET.SubElement(to, db.phrase, role="latex")
        phrase.text = tex
        dbeq.tail = equ.tail
    ...
    def transform_cell(self, elem):
        if self.cell_delegate is not None:
            return self.cell_delegate.transform_cell(elem)
        texts = []
        number = elem.get('number')
        txn = self.elem.find(ip.transformed)
        if txn:
            texts.append(self.transform_input(txn, number))
        else:
            texts.append(self.transform_input(self.elem.find(ip.input), 
                                              number))
        for tag in ('traceback', 'stdout', 'stderr', 'output'):
            el = self.elem.find(ip[tag])
            if el:
                xform = getattr(self, 'transform_%s' % tag)
                texts.append(xform(el, number))
        return texts
            

    def transform_block(self, block):
        """Transform an <ip:block> element to a <db:programlisting> element.
        """
        ...
        for cell in block:
            ...
            if type is None:
                texts.append(self.transform_cell(elem))
            elif type in ('input', 'transformed'):
        ...
        listing = ET.XML('<db:programlisting role="ipy_block">%s</db:programlisting>'
                    % '\n'.join(texts))

        # <ip:block>'s might be in mixed-content, so preserve the tail text
        listing.tail = block.tail
        return listing

    def transform_sheet(self, sheet, nodetype='article'):
        """Transform a <ip:sheet> element to a <db:section> element.
        """
        # XXX: Check again if copy.deepcopy() is working correctly
    ...
    def equation_images(cls, tree, name):
        ...
        if can_tex == False:
            ...
            equations = tree.xpath('//db:informalequation',
                namespaces=nsmap(db))
            equations.extend(tree.xpath('//db:inlineequation', 
                namespaces=nsmap(db)))
            for eq in equations:
                ...
                data = ET.SubElement(io, db.imagedata, 
