...
from notabene.xmlutils import nsmap, rdf, dc, xlink, ip, db
...
class SubelemGetter(SubelemWrapper):
    ...
    def __call__(self, ob):
        element = ob.element.find(ip[self.propname])
...
class Cell(object):
    ...
    def __init__(self, parent, number, element=None):
        ...
        if element is None:
            self.element = ET.SubElement(parent.element, ip.cell)
            self.number = number #sets the xml too via the property
        else:
            self.element = element
            assert self.number == number

    input  = property(SubelemGetter(ip.input),  SubelemSetter(ip.input))
    output = property(SubelemGetter(ip.output), SubelemSetter(ip.output))
    stdout = property(SubelemGetter(ip.stdout), SubelemSetter(ip.stdout))
    stderr = property(SubelemGetter(ip.stderr), SubelemSetter(ip.stderr))
    traceback = property(SubelemGetter(ip.traceback), SubelemSetter(ip.traceback))    
    ...
    def get_sheet_tags(self):
        yield ET.Element(ip.cell, number=str(self.number))
...
class Log(object):
    ...
    def __init__(self, parent, logid, element=None):
        ...
        if element is None:
            self.element = ET.SubElement(parent.root, ip.log, id=logid)
...
class Notebook(object):
    ...
    def __init__(self, name, root=None):
        ...
        if root is None:
            self.root = ET.Element(ip.notebook, version=self.version,
                nsmap=nsmap(ip, db))
    ...
    def check_errors(self):
        """Checks the notebook for syntax errors, returns (user-friendly) 
        reports.
        """
    ...
    def add_log(self, id='default-log'):
        ...
        id is a unique identifier. No other XML element in this file should 
        have the same id.
        """
    ...
    def add_meta(self, name, content, scheme=None):
        ...
        """
        meta = ET.SubElement(self.head, ip.meta, name=name, content=content)
    ...
    def add_figure(self, parent, filename, caption=None, type='png'):
        """Add a figure.
        """
        fig = ET.SubElement(parent, ip.figure, filename=filename, type=type)
        if caption is not None:
            fig.text = caption

    def add_equation(self, parent, tex, title=None):
        """Add an equation (in tex). If given, a title is set."""
        equ = ET.SubElement(parent, ip.equation, title=title, tex=tex)
    ...
    def default_sheet(self, logid='default-log'):
        log = self.logs[logid]
        sheet = ET.Element(ip.sheet)
        block = ET.SubElement(sheet, ip.block, logid=logid)
        for cell in log:
    ...
    def get_from_log(self, tag, number, logid='default-log'):
        ...
        xpath = '/ip:notebook/ip:log[@id="%s"]/ip:cell[@number="%s"]/ip:%s' % (logid,
            number, tag)
        elems = self.xpath.evaluate(xpath)
        if elems:
            return elems[0]
        else:
    ...
    def get_sheet(self):
        try:
            return self.xpath.evaluate('./ip:sheet')[0] #assumes a single sheet
        except IndexError:
    ...
    def get_title(self):
        if self.sheet is not None:
            ...
            try:
                title = self.sheet.xpath('./db:title', 
    ...
    def get_contents(self):
        if self.sheet is not None:
            ...
            sections = self.sheet.xpath('./db:section',
                namespaces=nsmap(db))
...
def book2docbook(bookfilename):
    ...
    for link in book.xpath('//*[@xlink:type="simple"]', 
        ...
        sheet = nb.xpath.evaluate('/ip:notebook/ip:sheet[@id="%s"]' % frag)
        if sheet == []:
            raise ValueError('could not find <ip:sheet id="%s"> in %s' %
                (frag, path))
        else:
            sheet = sheet[0]
        dbf = DBFormatter(nb)
