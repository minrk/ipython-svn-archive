...
class RemoteController(RemoteControllerBase):
    ...
    MAX_LENGTH = protocols.NetstringSocket.MAX_LENGTH
    ...
    def _sendSerialized(self, s, key):
        assert newserialized.ISerialized.providedBy(s)
        if s.getTypeDescriptor() == 'pickle':
            self.es.writeNetstring('PICKLE %s' % key)
            self.es.writeNetstring(s.getData())
        if s.getTypeDescriptor() == 'ndarray':
            self.es.writeNetstring('ARRAY %s' % key)
            md = s.getMetadata()
            self.es.writeNetstring(pickle.dumps(md['shape'], 2))
            self.es.writeNetstring(md['dtype'])
            self.es.writeNetstring(s.getData())
    ...
    def push(self, targets, *keys, **namespace):
        ...
        for key in namespace:
            value = namespace[key]
            try:
                serialObject = newserialized.serialize(value)
            except Exception, e:
        ...
        if string != "PUSH READY":
            ...
            return False
        for key in namespace:
            self._sendSerialized(serializedNamespace[key], key)
    ...
    def pull(self, targets, *keys):
        ...
        while string not in ['PULL OK', 'PULL FAIL']:
            ...
            if string_split[0] == "PICKLE":
                # if it's a pickle
                sPickle = newserialized.Serialized(self.es.readNetstring(), 'pickle')
                try:
                    data = newserialized.unserialize(sPickle)
                except Exception, e:
                    print "Error unpickling object: ", e
                    return False
            elif string_split[0] == 'ARRAY':
                shape = pickle.loads(self.es.readNetstring())
                dtype = self.es.readNetstring()
                rawdata = self.es.readNetstring()
                serial = newserialized.Serialized(rawdata, 'ndarray', 
                    {'shape':shape, 'dtype':dtype})
                try:
                    data = newserialized.unserialize(serial)
                except Exception, e:
                    print "Error unserializing ndarray: ", e
                    return False
            else:
    ...
    def scatter(self, targets, key, seq, style='basic', flatten=False):
        ...
        try:
            serial = newserialized.serialize(seq)
        except:
        ...
        self._sendSerialized(serial, key)
    ...
    def gather(self, targets, key, style='basic'):
        ...
        if split[0] == 'PICKLE':
            ...
            sPickle = newserialized.Serialized(self.es.readNetstring(), 'pickle')
            try:
                obj = newserialized.unserialize(sPickle)
            except Exception, e:
                print "Error unpickling object: ", e
                return False
        elif split[0] == 'ARRAY':
            ...
            serial = newserialized.Serialized(rawdata, 'ndarray', 
                {'shape':shape, 'dtype':dtype})
            try:
                obj = newserialized.unserialize(serial)
            except Exception, e:
                print "Error unserializing ndarray: ", e
                return False
        else:
    ...
    def notify(self, addr=None, flag=True):
        ...
        :Parameters:
         ...
         - `flag`: A boolean to turn notification on (``True``) or off 
           (``False``)
        """
        raise NotImplementedError("The notification interface has been disabled.")
        if not self._checkConnection():
            print "Not Connected"
            return False
...
class VanillaControllerProtocol(protocols.EnhancedNetstringReceiver):
    ...
    def handlePushing_PICKLE(self, package):
        ...
        self.workVars['pushDict'][self.workVars['pushKey']] = serial
    ...
    def pullOK(self, entireResultList):
        for perTargetResultList in entireResultList:
            ...
            for key, serialResult in zip(self.workVars['pullKeys'] ,perTargetResultList):
                if not newserialized.ISerialized.providedBy(serialResult):
                    try:
                        serialResult = newserialized.serialize(serialResult)
                    except:
                        return self.pullFail()
                self.sendSerialized(serialResult, key)
    ...
    def handleScatter_PICKLE(self, package):
        self.nextHandler = self.handleUnexpectedData
        serial = newserialized.Serialized(package, typeDescriptor='pickle')
        self.workVars['scatterDict'][self.workVars['scatterKey']] = serial
    ...
    def scatterCallback(self, _):
        ...
        obj = newserialized.unserialize(self.workVars['scatterDict'][key])
        kw = {}
    ...
    def gatherOK(self, obj):
        try:
            ...
            self.sendSerialized(serial, self.workVars['gatherKey'])
        except:
        ...
        for r in results:
            try:
                if isinstance(r, Failure):
                    serial = newserialized.serialize(r)
                    fail = True
                else:
                    serial = newserialized.serialize(r)
                    fail = False
    ...
    def getResultOK(self, result):
        try:
            s = newserialized.serialize(result)
        except Exception:
            self.getResultFinish("FAIL")
            return
        else:
            self.sendSerialized(s, 'RESULT')
    ...
    def statusOK(self, status):
        try:
            serial = newserialized.serialize(status)
        except pickle.PickleError:
...
class IVanillaControllerFactory(Interface):
    ...
    See the documentation for IEngineMultiplexer for details about these methods.
    """
...
class VanillaControllerFactoryFromService(protocols.EnhancedServerFactory):
    ...
    The methods here are those of the IEngineMultiplexer, and documentation
    can be found in the interface definition of IEngineMultiplexer.
    
    Where are the fooAll versions of the IEngineMultiplexer methods?
    """
    ...
    def __init__(self, service):
        ...
        assert IMultiEngine.providedBy(service), \
            "IMultiEngine not provided by " + repr(service)
